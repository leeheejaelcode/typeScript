...
// 변수 as 타입
function assertionArray(x: number | string) {
  let arr: number[] = [];
  arr[0] = x as number;
}
...
function numberArray(array: (number | string)[]): number[] {
  let result: number[] = array.map((x: number | string) => {
...
function lastSubject(subject: { subject: string | string[] }): string {
